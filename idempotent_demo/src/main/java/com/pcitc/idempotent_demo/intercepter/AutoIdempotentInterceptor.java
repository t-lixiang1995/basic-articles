package com.pcitc.idempotent_demo.intercepter;
...
public class AutoIdempotentInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (methodAnnotation != null) {
            try {
                Boolean result =  tokenService.checkToken(request);// 幂等性校验, 校验通过则放行, 校验失败则抛出异常, 并通过统一异常处理返回友好提示
                System.out.println(result);
                return result;
            }catch (Exception ex){
                RetResult<Object> failedResult = RetResultUtil.remark(101, ex.getMessage());
